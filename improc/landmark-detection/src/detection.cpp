 ...
 */
std::vector<std::pair<std::string, std::vector<std::pair<cv::Scalar, cv::Scalar>>>> get_color_masks() {
    // laughs in cpp types
    return {
        {"YELLOW", {
...
std::vector<std::vector<cv::Point>> get_contours_from_image(
    ...
    cones.images.emplace_back("4_" + color + "_GAUSSIAN", gaussian_blur);
    current = gaussian_blur;

    cv::Canny(current, canny, CANNY_THRESH_LOW, CANNY_THRESH_HIGH);
    cones.images.emplace_back("5_" + color + "_CANNY", canny);
    current = canny;

    auto kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2, 2));
    cv::dilate(current, dilated, kernel);
    cones.images.emplace_back("6_" + color + "_DILATED", dilated);
    current = dilated;

    cv::erode(current, eroded, kernel);
    cones.images.emplace_back("7_" + color + "_ERODED", eroded);
    current = eroded;

    std::vector<std::vector<cv::Point>> contours;
    std::vector<cv::Vec4i> hierarchy;
    cv::findContours(
            current, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE
        );
    return contours;
}
...
cone_info find_cones(const cv::Mat& image) {
    ...
    cones.images.emplace_back("2_HSV", hsv);
    ...
    for(int i = 0; i < color_masks.size(); i++) {
        ...
        cones.images.emplace_back("3_" + color + "_MASK", masked_images[i]);
