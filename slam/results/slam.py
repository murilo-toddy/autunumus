...
from lib import get_cylinders_from_scan, write_cylinders,\
...
class Particle:
    ...
    def H_Ql_jacobian_and_measurement_covariance_for_landmark(
        ...
        Ql = np.dot(np.dot(H, self.landmark_covariances[landmark_number]), H.T) + Qt_measurement_covariance
        return (H, Ql)
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        # --->>> Insert your new code here.
        # Hints:
        min_ang, max_ang = LegoLogfile.min_max_bearing()
        for i in range(len(self.landmark_positions)):
            d, ang = self.h_expected_measurement_for_landmark(i, scanner_displacement)
...
if __name__ == '__main__':
    ...
    # Filter constants.
    control_motion_factor = 0.2  # Error in motor control.
    control_turn_factor = 0.6  # Additional error due to slip when turning.
    measurement_distance_stddev = 1.5  # Distance measurement error of cylinders.
    measurement_angle_stddev = 2 / 180.0 * pi  # Angle measurement error.
